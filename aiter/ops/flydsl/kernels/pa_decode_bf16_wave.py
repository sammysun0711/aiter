...
def compile_pa_decode_bf16_wave(head_dim, num_partitions, softmax_scale):
    ...
    def pa_decode_wave32_kernel(
        ...
        def _page(tt):
            page = fx.Int32(buf_scalar_load(tables, seq * max_blocks + tt))
            # CUDA-graph padded requests and freshly extended block tables may
            # expose -1 sentinels in slots that are masked by the logical
            # sequence state. Never turn those sentinels into negative global
            # memory addresses while asynchronously staging a whole page.
            page = (page >= fx.Int32(0)).select(page, fx.Int32(0))
