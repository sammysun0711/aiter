    ...
    *,
    ...
    if True:
        ...
        def _emit_moe_gemm1(
            ...
            if const_expr(persistent and not split_primary):
                ...
                loop_end = tiles_per_block
            else:
                c_pm = arith.constant(persist_m, index=True)
                loop_end = c_pm

            # Per-iteration state is rebound by _persist_iter so the closures
            # below see the tile assigned by the current scheduler iteration.
            ...
            if const_expr(split_primary):
                ...
                @flyc.jit
                def _run_persist():
                    # init=[] preserves the index-typed induction variable.
