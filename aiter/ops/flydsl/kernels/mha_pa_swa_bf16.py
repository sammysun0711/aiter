...
# Ported from tingqli/pyhip@1b0d5280774de7d3409e69dab1fb54732d6a219a.
...
def _k_load(tensor, lane, tile, physical, dq, hk, hkv, bn):
    page_offset = (fx.Int64(physical) * hk + fx.Int64(hkv)) * PAGE * dq
    resource = _resource(
        fx.make_view(
            fx.get_iter(tensor) + page_offset,
            fx.make_layout(PAGE * dq, 1),
        ),
        PAGE * dq * 2,
    )
    parts = []
...
def _v_load(tensor, lane, tile, physical, hk, hkv, bn):
    page_offset = (fx.Int64(physical) * hk + fx.Int64(hkv)) * PAGE * DV
    resource = _resource(
        fx.make_view(
            fx.get_iter(tensor) + page_offset,
            fx.make_layout(PAGE * DV, 1),
        ),
        PAGE * DV * 2,
    )
...
def _swa_kernel(
    ...
    if qstart < q_len:
        ...
        if first < end:
            ...
            k = _k_load(K, lane, first, page, DQ, HK, hkv, BN)
            vw = _v_load(V, lane, first, page, HK, hkv, BN)
...
def _swa32_kernel(
    ...
    if qstart < q_len:
        ...
        for tile in range(first, end, fx.Int32(BN)):
            ...
            kw = _k_load(K, lane, tile, page, DQ, HK, hkv, BN)
            wait(vmcnt=0)
