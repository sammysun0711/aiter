...
from aiter.ops.triton.utils._triton.arch_info import get_arch, is_moe_a16w4_avail
...
_QWEN3_8_TP8_CONFIG_PATH = (
    Path(AITER_TRITON_CONFIGS_PATH) / "moe/gfx942-MOE-A16W4-QWEN3_8-TP8.json"
)


@lru_cache(maxsize=1)
def _load_qwen3_8_tp8_dispatch() -> dict[str, dict]:
    with _QWEN3_8_TP8_CONFIG_PATH.open(encoding="utf-8") as config_file:
        config = json.load(config_file)
    return config["single_layout_production_dispatch"]


def _triton_version_tuple() -> tuple[int, int]:
    version = triton.__version__.split("+", 1)[0].split(".")
    return int(version[0]), int(version[1])


def _is_qwen3_8_config_compatible(stage: str, config: dict) -> bool:
    # The supplied table was tuned with Triton 3.7. On gfx942, Triton 3.5 fails
    # LLVM lowering for its BLOCK_K=32/64 entries. Locally retuned BLOCK_K=128
    # entries compile on Triton 3.5, so keep those eligible.
    if get_arch() == "gfx942" and _triton_version_tuple() < (3, 7):
        if int(config["block_k"]) < 128:
            return False
    return True


def get_qwen3_8_tp8_kernel_config(m, n, k, routing_data):
    """Return a tuned gfx942 config for the Qwen3.8 TP8 local MoE shape."""
    if (
        get_arch() != "gfx942"
        or routing_data.n_expts_tot != 512
        or routing_data.n_expts_act != 10
    ):
        return None
    ...
    if m % routing_data.n_expts_act != 0:
        return None
    token_m = m // routing_data.n_expts_act
    block_m = routing_data.block_m
    dispatch = _load_qwen3_8_tp8_dispatch()

    exact = dispatch.get(str(token_m))
    if (
        exact is not None
        and int(exact[stage]["block_m"]) == block_m
        and _is_qwen3_8_config_compatible(stage, exact[stage])
    ):
        return dict(exact[stage])

    compatible = [
        (abs(int(candidate_m) - token_m), int(candidate_m), entry[stage])
        for candidate_m, entry in dispatch.items()
        if int(entry[stage]["block_m"]) == block_m
        and _is_qwen3_8_config_compatible(stage, entry[stage])
    ]
    if not compatible:
        return None
    return dict(min(compatible, key=lambda item: (item[0], item[1]))[2])


def get_kernel_config(m, n, k, routing_data):
    tuned_config = get_qwen3_8_tp8_kernel_config(m, n, k, routing_data)
...
def moe_gemm_a16w4(
    ...
    """
    assert is_moe_a16w4_avail(), "A16W4 MoE is not available on this device"
    ...
    config = (
        get_kernel_config(M, N, K, routing_data)
        if kernel_config is None
        else dict(kernel_config)
    )
    assert int(config["block_m"]) == block_m, (
        f"kernel config block_m={config['block_m']} does not match "
        f"routing block_m={block_m}"
    )
