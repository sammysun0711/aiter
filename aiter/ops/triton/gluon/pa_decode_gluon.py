...
def paged_attention_decode_v2_gluon_large_block_dot_kernel(
    ...
    for kv_block_index in gl.static_range(KV_COMPUTE_BLOCK_COUNT):
        ...
        key_head_mask = (
            head_size_split_offsets[:, None, None]
            * CONTIGUOUS_KV_ELEMENTS_16B_LOAD
            + contiguous_kv_elements_offsets[None, None, :]
            < head_size
        )
...
def paged_attention_decode_sliding_window_head_1(
    ...
    if QUERY_SEQ_LEN_POW2 == 1:
        ...
        value_head_size_layout: gl.constexpr = gl.SliceLayout(
            0, blocked_query_layout
        )
    else:
        mtp_query_len_layout: gl.constexpr = gl.SliceLayout(
            1, gl.SliceLayout(2, mtp_blocked_query_layout)
    ...
    if ONE_SHOT:
        if QUERY_SEQ_LEN_POW2 == 1:
            output_mask = (
                query_group_size_offsets[:, None] < query_group_size
            ) & (value_head_size_offsets[None, :] < value_head_size)
        else:
            output_mask = (
                (
                    mtp_idx * QUERY_SEQ_LEN_POW2
                    + mtp_query_len_offsets[:, None, None]
                    < query_seq_len
                )
                & (
                    mtp_query_group_size_offsets[None, :, None]
    ...
    for sequence_partition_idx in range(
        ...
        if VALUE_TRANSPOSED:
            ...
            value_head_mask = (
                value_dim2_offsets[None, None, :, None] < value_head_size
            )
        ...
        # A persistent program owns only [sequence_start_idx,
        # sequence_end_idx), while causal visibility is defined against the
        # global context_length. Keep the local split bounds and global
        # qlen-dependent frontier as separate conditions.
...
def paged_attention_decode_sliding_window(
    ...
    for sequence_partition_idx in range(
        ...
        if VALUE_TRANSPOSED:
            ...
            if KV_BLOCK_SIZE > CONTEXT_PARTITION_SIZE and SLIDING_WINDOW > 0:
                ...
                value_tensor = gl.load(
                    value_cache_ptr + value_block_offsets,
                    mask=value_head_mask
                    & value_in_window_mask[None, None, :],
                    other=0.0,
                    cache_modifier=".cg",
                )
            else:
        ...
        # Apply causal masking if required
        # `sequence_end_idx` bounds this persistent split; context_length is
        # the global causal frontier shared by all splits.
        if IS_CAUSAL:
